<?php
    ...
    if(mysqli_num_rows($resultgetbooks)!=0)
    {
        ...
        {
            // echo 'https://www.googleapis.com/books/v1/volumes?q=isbn:'.$row2["ISBN"].'&key='.;
            $response = curl_get_contents('https://www.googleapis.com/books/v1/volumes?q=isbn:'.$row2["ISBN"].'&key='.$api_key);
            // echo $response;
            $response = json_decode($response);
            if ($response->totalItems == 0)
            {
                $sql_del_book = "DELETE FROM books WHERE ISBN='".$row2["ISBN"]."'";
                if(mysqli_query($conn,$sql_del_book))
                {
                    echo $row2["BookTitle"]." deleted.";
                    echo "<br/>";
                }
                $final_num++;
            }
    ...
    echo "Final number of deleted books:".$final_num;
    mysqli_close($conn);
?>